...
If you need a compact preset note for keeping the first web-app pass to one form, one primary action, and one validator-backed result card, open `docs/PRESET_WEB_APP_ONE_FORM_RESULT_CARD_NOTE.md`.
...
If you need the matching governance handoff for longer proposal markdown loaded through `proposal_copy_file`, open `docs/PRESET_GOVERNANCE_PROPOSAL_COPY_FILE_NOTE.md`.
...
If you need the matching compact reminder that the same phase-one web-app slice should keep one scenario file, one result card, and one report download status cue together, open `docs/PRESET_WEB_APP_GOVERNANCE_REPORT_DOWNLOAD_STATUS_NOTE.md`.
...
If you need a compact UI note for keeping that same first governance web-app proof tied to one scenario file and one visible `report.outputs.files` artifact map, open `docs/PRESET_WEB_APP_GOVERNANCE_REPORT_OUTPUT_FILES_NOTE.md`.
...
If you need a compact recheck note for that same chooser/result-card/export slice before commit or push, open `docs/PRESET_JSON_RESULT_CARD_EXPORT_RECHECK_NOTE.md`.
...
If you need a compact note for keeping the first result card readable with one setup command, one validation command, and one export target, open `docs/PRESET_JSON_RESULT_CARD_SETUP_VALIDATE_EXPORT_TRIO_NOTE.md`.
...
If you need a compact governance-web-demo note for keeping one scenario source path and one report basename visible on that same first result card, open `docs/PRESET_WEB_APP_GOVERNANCE_SOURCE_PATH_REPORT_BASENAME_NOTE.md`.
...
If you need a compact note for keeping the first result card readable before any preset is selected, open `docs/PRESET_JSON_RESULT_CARD_EMPTY_STATE_NOTE.md`.
...
If you need a compact owner-facing note for keeping the chosen preset, owner-ready status, and export target visible together on that same first result card, open `docs/PRESET_JSON_RESULT_CARD_OWNER_EXPORT_NOTE.md`.
...
If you need a compact maintainer note for the repo-4/repo-5 phase-one status lane, open `docs/PRESET_REPO45_PHASE_ONE_STATUS_NOTE.md`.
...
If you need a compact maintainer recheck before claiming that same result-card slice is ready to validate and export, open `docs/PRESET_WEB_APP_RESULT_CARD_VALIDATE_EXPORT_RECHECK.md`.
...
If you need the matching compact reminder to keep the cross-repo maintainer update to one short validated status line per repo, open `docs/PRESET_WEB_APP_ONE_LINE_RESULT_STATUS_NOTE.md`.
If you need a compact preset note that the first web-app slice should still land one validated result in every repo-4/repo-5 pass, open `docs/PRESET_WEB_APP_REPO45_EVERY_PASS_NOTE.md`.
If you need the shortest note for keeping one exported preset JSON file tied to one setup form and one result-card validation pass, open `docs/PRESET_JSON_FORM_RESULT_CARD_VALIDATE_NOTE.md`.
If you need the shortest recovery-first reminder for replaying that same preset-backed form -> primary action -> result-card validation loop, open `docs/PRESET_JSON_FORM_RESULT_CARD_VALIDATE_RECOVERY_NOTE.md`.
...
If you need a compact UI note for keeping setup command, validation command, and export path visible together on that same first result card, open `docs/PRESET_WEB_APP_RESULT_CARD_COMMAND_PATH_NOTE.md`.
...
If you need the matching concise note that keeps repo 4 and repo 5 explicitly tied to validation state before any commit/push claim, open `docs/PRESET_FIVE_REPO_REPO45_VALIDATE_STATUS_NOTE.md`.
