...
class InitProjectTests(unittest.TestCase):
    ...
    def test_showcase_alias_resolves_to_web_app(self) -> None:
    ...
    def test_result_card_demo_alias_resolves_to_web_app(self) -> None:
        from oss_launchpad_cli.cli import _resolve_preset_name

        self.assertEqual(_resolve_preset_name("result-card-demo"), "web-app")

    def test_governance_result_demo_alias_resolves_to_web_app(self) -> None:
        from oss_launchpad_cli.cli import _resolve_preset_name

        self.assertEqual(_resolve_preset_name("governance-result-demo"), "web-app")
    ...
    def test_readme_mentions_web_app_primary_cta_note(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/PRESET_WEB_APP_PRIMARY_CTA_NOTE.md", readme)
        self.assertTrue((ROOT / "docs" / "PRESET_WEB_APP_PRIMARY_CTA_NOTE.md").exists())

    def test_readme_mentions_web_app_result_card_export_rule(self) -> None:
        readme = (ROOT / 'README.md').read_text(encoding='utf-8')
    ...
    def test_readme_mentions_web_app_form_result_export_check(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/PRESET_WEB_APP_FORM_RESULT_EXPORT_CHECK.md", readme)
        self.assertTrue((ROOT / "docs" / "PRESET_WEB_APP_FORM_RESULT_EXPORT_CHECK.md").exists())
    ...
    def test_readme_mentions_web_app_one_action_check(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/PRESET_WEB_APP_ONE_ACTION_CHECK.md", readme)
        self.assertTrue((ROOT / "docs" / "PRESET_WEB_APP_ONE_ACTION_CHECK.md").exists())
    ...
    def test_readme_mentions_web_app_form_result_card_check(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/PRESET_WEB_APP_FORM_RESULT_CARD_CHECK.md", readme)
        self.assertTrue((ROOT / "docs" / "PRESET_WEB_APP_FORM_RESULT_CARD_CHECK.md").exists())
    ...
    def test_build_next_steps_keeps_web_app_bootstrap_handoffs_visible(self) -> None:
        ...
        self.assertIn("Use docs/PRESET_WEB_APP_PLAYWRIGHT_STABILITY_LANE.md before widening browser automation so the first form-to-card proof stays reproducible.", steps)
